def gene_group_split(target_filename,target_id_column,gene_group_list,gene_group_id_column,header,split_line_num):
	...
	l=0
	for gene_group in gene_group0: ### start a new gene group
		j=j+1
		for h in range(0,split_line_num):
			...
			target0_genegroup.write('\n')
			target0_genegroup_colorsplit.write(str(j)+'\t'+'split_line'+'\t'+str(l)+'\n')
			l=l+1
		...
		class2name={1:'RP',2:'SAGA',3:'TFIID',4:'CUTs',5:'SUTs',6:'XUTs',7:'tRNA',8:'X_element'}
		## write data
		if header=='T':
			start_row=1
		else:
		...
		for records in target0[0:]:
			...
			if records[target_id_column] in gene_group1:
				...
				for rec in records:
					target0_genegroup.write(rec+'\t')
				target0_genegroup.write('\n')
				target0_genegroup_colorsplit.write(str(j)+'\t'+class2name[j]+'\t'+str(l)+'\n')
				l=l+1
			else:
