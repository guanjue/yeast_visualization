		...
		if len(records.split())!=0:
				...
				if roman2arabic_tf =='T':
					...
					if tmp_inf[0] in roman2arabic: ### remove header & convert roman to arabic number
						...
						if tmp_inf[5] == '+' and int(tmp_inf[1]) - upstream_expan >=0:
							expand_inf = [roman2arabic[tmp_inf[0]] ] + [ int(tmp_inf[1])-upstream_expan, int(tmp_inf[2])+downstream_expan+1, tmp_inf[3], tmp_inf[4], tmp_inf[5], int(tmp_inf[2])-int(tmp_inf[1]) ]
						elif tmp_inf[5] == '-' and int(tmp_inf[1]) - downstream_expan >=0:
							expand_inf = [roman2arabic[tmp_inf[0]] ] + [ int(tmp_inf[1])-downstream_expan, int(tmp_inf[2])+upstream_expan+1, tmp_inf[3], tmp_inf[4], tmp_inf[5], int(tmp_inf[2])-int(tmp_inf[1]) ]
						else:
							end_num = end_num + 1
					...
					if tmp_inf[0][0:3] =='chr': ### remove header
						...
						if tmp_inf[5] == '+' and int(tmp_inf[1]) - upstream_expan >=0:
							expand_inf = [tmp_inf[0], int(tmp_inf[1])-upstream_expan, int(tmp_inf[2])+downstream_expan+1, tmp_inf[3], tmp_inf[4], tmp_inf[5], int(tmp_inf[2])-int(tmp_inf[1]) ]
						elif tmp_inf[5] == '-' and int(tmp_inf[1]) - downstream_expan >=0:
							expand_inf = [tmp_inf[0], int(tmp_inf[1])-downstream_expan, int(tmp_inf[2])+upstream_expan+1, tmp_inf[3], tmp_inf[4], tmp_inf[5], int(tmp_inf[2])-int(tmp_inf[1]) ]
						data01.append(expand_inf)
