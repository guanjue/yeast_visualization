		...
		if len(records.split())!=0:
				...
				if roman2arabic_tf =='T':
					...
					if tmp_inf[0] in roman2arabic: ### remove header & convert roman to arabic number
						...
						if tmp_inf[7] == '+' and int(tmp_inf[3]) - upstream_expan >=0:
							expand_inf = [roman2arabic[tmp_inf[0]] ] + [ int(tmp_inf[3])-upstream_expan, int(tmp_inf[4])+downstream_expan, tmp_inf[8], tmp_inf[5], tmp_inf[6], int(tmp_inf[4])-int(tmp_inf[3]) ]
						elif tmp_inf[7] == '-' and int(tmp_inf[3]) - downstream_expan >=0:
							expand_inf = [roman2arabic[tmp_inf[0]] ] + [ int(tmp_inf[3])-downstream_expan, int(tmp_inf[4])+upstream_expan, tmp_inf[8], tmp_inf[5], tmp_inf[6], int(tmp_inf[4])-int(tmp_inf[3]) ]
						else:
							end_num = end_num + 1
					...
					if tmp_inf[0][0:3] =='chr': ### remove header
						...
						if tmp_inf[7] == '+' and int(tmp_inf[3]) - upstream_expan >=0:
							expand_inf = [tmp_inf[0], int(tmp_inf[3])-upstream_expan, int(tmp_inf[4])+downstream_expan, tmp_inf[8], tmp_inf[5], tmp_inf[6], int(tmp_inf[4])-int(tmp_inf[3]) ]
						elif tmp_inf[7] == '-' and int(tmp_inf[3]) - downstream_expan >=0:
							expand_inf = [tmp_inf[0], int(tmp_inf[3])-downstream_expan, int(tmp_inf[4])+upstream_expan, tmp_inf[8], tmp_inf[5], tmp_inf[6], int(tmp_inf[4])-int(tmp_inf[3]) ]
						data01.append(expand_inf)
