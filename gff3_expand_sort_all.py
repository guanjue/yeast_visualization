...
def gff3_expand_sort(file_name,upstream_expan_list,downstream_expan_list,roman2arabic_tf,outputname):
		...
		if len(records.split())!=0:
				...
				if roman2arabic_tf =='T':
					...
					if tmp_inf[0] in roman2arabic: ### remove header & convert roman to arabic number
						name=tmp_inf[8].split(';')[0].split('=')[1].split('%')[0]
...
def main(argv):
	...
	gff3_expand_sort(file_name, upstream_expan_list, downstream_expan_list, roman2arabic_tf,outputname)
