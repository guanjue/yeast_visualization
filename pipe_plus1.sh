...
while read info; do
	...
	#####################################
	echo 'bound genes'
	python extract_bound.py -b $id'_Xu_2009_ORF-Ts_V64.genelist.txt' -t Yeast_plus_one_sacCer3.gff -o $id'_Yeast_plus_one_sacCer3.bed'
	...
	sort -k10,10nr $id'_Yeast_plus_one_sacCer3.bound.txt' | awk -F '\t' -v OFS='\t' '{if ($6=="+") print $1,$2-500,$2+100,$4,$5,$6; else print $1,$2-100,$2+500,$4,$5,$6}' > $id'_Yeast_plus_one_sacCer3.bound.sort.bed'
	echo 'get heatmap table'
	java -jar TagPileup.jar -b $bam_file'.bam' -i $bam_file'.bam.bai' -c $id'_Yeast_plus_one_sacCer3.bound.sort.bed' -s 6 -n 5 -e true -r 0 -p true -a 1 -t 4 -w 0 -h true -m true -o ./ -x $id'_Yeast_plus_one_sacCer3.bound.sort.composite.txt'
	echo 'generate heatmap'
	Rscript heatmap.R $id'_Yeast_plus_one_sacCer3_'$bam_file'_read1_combined.tabular' $id'_Yeast_plus_one_sacCer3_'$bam_file'_read1_combined.png'
